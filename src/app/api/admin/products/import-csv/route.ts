...
const importCsvHandler: AdminApiHandler = async (req) => {
  try {
    ...
    for (const [index, row] of (rows as any[]).entries()) {
      try {
        ...
        if (actualFileName) {
          const {
            data: { publicUrl },
          } = supabase.storage.from('gold-nexus-images').getPublicUrl(actualFileName);
        ...
        const createData: typeof productData & { imageUrl: string } = {
          ...productData,
    ...
    if (validUpdates.length > 0) {
      try {
        await prisma.$transaction(validUpdates.map((op) => prisma.product.upsert(op)));
